...
class InvitationServiceImplTest : BehaviorSpec({
    ...
    given("createInvitation - inviterAvatar를 찾을 수 없을 때") {
        `when`("createInvitation을 호출하면") {
            then("NOT_FOUND_AVATAR 예외가 발생하고 inviteeAvatar 조회는 수행되지 않는다") {
                ...
                val ex = shouldThrow<SimulationException> {
                    sut.createInvitation(memberId, inviterAvatarId, inviteeAvatarId, "메시지")
                }

                ex.errorCode shouldBe SimulationErrorCode.NOT_FOUND_AVATAR
    ...
    given("createInvitation - inviterAvatar의 소유자가 아닐 때") {
        `when`("createInvitation을 호출하면") {
            then("NOT_AVATAR_OWNER 예외가 발생하고 inviteeAvatar 조회는 수행되지 않는다") {
                ...
                ex.errorCode shouldBe SimulationErrorCode.NOT_AVATAR_OWNER
                verify(exactly = 0) { avatarService.getAvatarById(inviteeAvatarId) }
            }
        }
    }

    given("createInvitation - inviteeAvatar를 찾을 수 없을 때") {
        `when`("createInvitation을 호출하면") {
    ...
    given("createInvitation - inviter에게 진행 중인 시뮬레이션이 있을 때") {
        `when`("createInvitation을 호출하면") {
            then("IN_PROGRESS_SIMULATION 예외가 발생하고 메시지에 inviterAvatar.name이 포함된다") {
                val memberId = UUID.randomUUID()
                val inviterAvatarId = UUID.randomUUID()
                val inviteeAvatarId = UUID.randomUUID()
                ...
                every {
                    invitationRepository.findSimulationInfoByStatusesAndAvatars(
                ...
                ex.errorCode shouldBe SimulationErrorCode.IN_PROGRESS_SIMULATION
                ex.message!! shouldContain inviterName
            }
        }
    }

    given("createInvitation - invitee에게만 진행 중인 시뮬레이션이 있을 때") {
        `when`("createInvitation을 호출하면") {
            then("IN_PROGRESS_SIMULATION 예외가 발생하고 메시지에 inviteeAvatar.name이 포함된다") {
                val memberId = UUID.randomUUID()
                val inviterAvatarId = UUID.randomUUID()
                val inviteeAvatarId = UUID.randomUUID()
    ...
    given("acceptInvitation - 시뮬레이션 초대를 찾을 수 없을 때") {
        `when`("acceptInvitation을 호출하면") {
            then("NOT_FOUND_SIMULATION_INVITATION 예외가 발생한다") {
                val memberId = UUID.randomUUID()
                ...
                ex.errorCode shouldBe SimulationErrorCode.NOT_FOUND_SIMULATION_INVITATION
            }
        }
    }
    ...
    given("acceptInvitation - 초대받은 사용자가 아닐 때") {
        `when`("acceptInvitation을 호출하면") {
            then("NOT_INVITATION_RECIPIENT 예외가 발생하고 accept는 호출되지 않는다") {
                ...
                val invitation = mockk<SimulationInvitation>()
                ...
                ex.errorCode shouldBe SimulationErrorCode.NOT_INVITATION_RECIPIENT
    ...
    given("acceptInvitation - 초대 상태가 PENDING이 아닐 때") {
        listOf(
            ...
            `when`("초대 상태가 $status 일 때 acceptInvitation을 호출하면") {
                then("INVALID_SIMULATION_INVITATION_STATUS 예외가 발생하고 메시지에 상태와 액션이 포함된다") {
                    val memberId = UUID.randomUUID()
                    val invitationId = UUID.randomUUID()
    ...
    given("rejectInvitation - 시뮬레이션 초대를 찾을 수 없을 때") {
    ...
    given("cancelInvitation - 시뮬레이션 초대를 찾을 수 없을 때") {
    ...
    given("cancelInvitation - 초대한 사용자가 아닐 때") {
        `when`("cancelInvitation을 호출하면") {
            then("NOT_INVITATION_CREATOR 예외가 발생하고 cancel은 호출되지 않는다") {
                ...
                ex.errorCode shouldBe SimulationErrorCode.NOT_INVITATION_CREATOR
    ...
    given("getInvitationHistory - 유효하지 않은 cursor 문자열이 주어질 때") {
        `when`("getInvitationHistory를 호출하면") {
            then("INVALID_CURSOR 예외가 발생한다") {
                ...
                ex.errorCode shouldBe SimulationErrorCode.INVALID_CURSOR
            }
        }
    }
    ...
    given("createInvitation - 정상적인 시뮬레이션 초대 생성 요청일 때") {
        `when`("createInvitation을 호출하면") {
            then("invitationRepository.save가 1회 호출되고 inviteeAvatar.id가 올바르게 저장되며 응답 status가 PENDING이다") {
                ...
                val invitationSlot = slot<SimulationInvitation>()
